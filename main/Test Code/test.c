...
int main(int argc, char *argv[]){
	...
	if (argc != 2){
		printf("Error, please enter name of file to open!\n");
		return 1;
	}
	...
	com = (GCommand*)malloc(sizeof(GCommand));
	int count = 0;
	
	while (fgets(fileline, 150, pfile) != NULL){	
		count++;
		if (fileline[0] == ';'){
			printf("%s", fileline);
		} else {
			printf("%s", fileline);
			i = readGCodeString(fileline, com);
			printf("%c%f ", com->letter, com->num);
			if (com->xf == 1){
				printf("X = %f ", com->x);
			}
			if (com->yf == 1){
